...
class JukeboxPanelLinuxBinaryModule(JukeboxPanelInputBase, JukeboxPanelOutputBase):
    ...
    def WriteToThreeDigitDisplay(self, message: str):
        self._write_raw(JBP_TARGET_3DIGIT, _pack_text(message))
    ...
    def WriteToFourDigitDisplay(self, message: str):
        self._write_raw(JBP_TARGET_4DIGIT, _pack_text(message))
    ...
    def _write_raw(self, target: int, segments: int):
        if target == JBP_TARGET_4DIGIT:
            segments |= self._led_bits()
        self._send(JBP_CMD_SET_RAW, target, segments & 0xFFFFFFFF)
