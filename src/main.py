...
from observers import UpdateEventType, ObserverBase, TerminalObserver, Coordinator, SingleLineObserver, SingleLineAnimatedSimpleObserver
...
async def main():
    ...
    vfd_song_title_observer = SingleLineAnimatedSimpleObserver(driver=vfdLine1, event_type=UpdateEventType.SONG_TITLE)
    vfd_song_title_observer.changeAnimation((RandomTypeWriter()))
    coorinator.add_observer(vfd_song_title_observer)

    vfd_artist_observer = SingleLineAnimatedSimpleObserver(driver=vfdLine0, event_type=UpdateEventType.ARTIST)
