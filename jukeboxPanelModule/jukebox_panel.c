...
static int keypad_debounce_ms = 50; /* matches JukeboxPanel::GetKey()'s 50ms settle time */
 ...
 * 16MHz AVR this protocol was designed around. */
 ...
 * bit, 32 data bits LSB-first, then 4 trailing bits to latch. */
...
/* Debounce state, mirroring JukeboxPanel::GetKey(): a key must read the
 * same non-'_' value continuously for keypad_debounce_ms before it is
 * reported, and is reported exactly once per press (not repeated while
 * held). */
...
static int keypad_scan_thread_fn(void *unused)
{
	...
	while (!kthread_should_stop()) {
		...
		if (this_key != last_key) {
			...
			if (this_key != '_' && this_key != last_sent)
